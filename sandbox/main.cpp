...
int main()
{
	...
	for (std::size_t index = 0; index < collectibles.size(); ++index) {
		...
		collectibles[index].Add<Iryven::Transform>(Iryven::Transform{
			...
			.rotation = glm::quat(glm::angleAxis(
									glm::radians(25.0f),
									glm::vec3(0.0f, 0.0f, 1.0f)
								)),
			.scale = glm::vec3{ 0.35f },
			});
		collectibles[index].Add<Iryven::MeshRenderer>(duckModel);
	...
	for (auto& collectibleEntity : collectibles) {
		auto collider = Iryven::Collider::Sphere(0.35f);
	...
	world.AddSystem<Collectible, Iryven::Transform>(
		"Rotate Collectible",
		[](float delta, Collectible&, Iryven::Transform& collectibleTransform) {
			collectibleTransform.rotation = glm::normalize(
					glm::angleAxis(
					glm::radians(45.0f) * delta,
					glm::vec3(0.0f, 1.0f, 0.0f)) * collectibleTransform.rotation);
		});
